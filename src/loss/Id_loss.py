...
class Id_Loss(nn.Module):
    ...
    def __init__(self, opt, part, feature_length):
        ...
        self.global_swap = classifier(feature_length,opt.class_num*2)
    ...
    def forward(self, image_embedding_local, text_embedding_local, label,gs = False):
        criterion = nn.CrossEntropyLoss(reduction='mean')
        if gs:
            a = torch.cat((image_embedding_local,text_embedding_local),dim=0).squeeze()
            predict = self.global_swap(a)
